	...
	void*         *ThreadData      /*=NULL*/)
{
	...
	//创建信号量多线程同步使用）
	/*在信号量上我们定义两种操作： Wait等待） 和 Release释放）。
	当一个线程调用Wait操作时，它要么得到资源然后将信号量减一，要么一直等下去指放入阻塞队列），
	直到信号量大于等于一时。Release释放）实际上是在信号量上执行加操作*/
	...
	//创建事件为手动置位，一次只能进入一个，False为初始不是运行状态多线程同步使用）
